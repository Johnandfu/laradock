...
if [[ -z "$KAFKA_LISTENERS_KRAFT" ]]; then
  echo 'Using default listeners'
else
  echo "Using listeners: ${KAFKA_LISTENERS_KRAFT}"
  sed -r -i "s@^#?listeners=.*@listeners=$KAFKA_LISTENERS_KRAFT@g" "/opt/kafka/config/kraft/server.properties"
fi
